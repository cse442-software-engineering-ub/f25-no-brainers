  ...
  const fetchListings = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE}/seller-dashboard/manage_seller_listings.php`,
        {
          method: "POST",
          headers: {
      ...
      if (result.success) {
        ...
        const itemsWithAccepted = transformedListings.filter(
          (l) => l.has_accepted_scheduled_purchase
        );
        if (itemsWithAccepted.length > 0) {
          console.log(
            "Items with accepted scheduled purchases:",
            itemsWithAccepted
          );
        }
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            {getSortedListings().map((listing) => (
              <div
                key={listing.id}
                className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border dark:border-gray-700 p-4 sm:p-6"
              >
                  ...
                  <div className="flex items-center justify-between sm:justify-end space-x-3">
                    ...
                    <button
                      onClick={() => {
                        ...
                        setPendingStatusValue(listing.status || "Active");
                        setStatusOpen(true);
                      }}
                      disabled={
                      ...
                      title={
                        listing.has_accepted_scheduled_purchase === true
                          ? "Items with an active Scheduled Purchase cannot be modified"
                          : ""
                      }
                    >
                      Set Status
                    ...
                    <button
                      onClick={() =>
                        navigate(`/app/product-listing/edit/${listing.id}`)
                      }
                      disabled={
                        listing.has_accepted_scheduled_purchase === true
                      }
                ...
                onClick={async () => {
                  try {
                    ...
                    const res = await fetch(
                      `${BASE}/seller-dashboard/set_item_status.php`,
                      {
                        method: "POST",
                        headers: {
                          "Content-Type": "application/json",
                    ...
                    if (!res.ok || !result.success)
                      throw new Error(
                        result.error || `Status update failed (${res.status})`
                      );

                    const next = listings.map((l) =>
                      l.id === pendingStatusId
                        ? { ...l, status: pendingStatusValue }
                        : l
                    );
                    setListings(next);
