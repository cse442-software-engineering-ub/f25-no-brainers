...
function OngoingPurchasesPage() {
    ...
    if (req.is_trade && req.trade_item_description) {
      return (
        <div
          ...
          } border-4 rounded-lg p-4 my-3 shadow-lg`}
        >
      ...
      return (
        <div
          className={`${
            ...
            isInactive ? "border-red-400" : "border-emerald-500"
          } rounded-lg p-5 my-3 shadow-lg`}
        >
          ...
          <p className="text-4xl font-bold text-white">
            ${Number(req.negotiated_price).toFixed(2)}
          </p>
        </div>
      );
    } else if (
      ...
      return (
        <div
          className={`${
            ...
            isInactive ? "border-red-400" : "border-blue-500"
          } rounded-lg p-5 my-3 shadow-lg`}
        >
          <div className="flex items-center gap-2 mb-2">
            <svg
          ...
          <p className="text-4xl font-bold text-white">
            ${Number(req.item.listing_price).toFixed(2)}
          </p>
        </div>
      );
    }
  ...
  const PurchaseCard = ({ req, perspective }) => {
    ...
    return (
      <div
        className={`${cardBgColor} border-2 ${cardBorderColor} rounded-lg p-5 shadow-sm ${
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              ...
              >
                ...
                {isBuyer
                  ? `${req.seller?.first_name || ""} ${
                      req.seller?.last_name || ""
                    }`.trim()
                  : `${req.buyer?.first_name || ""} ${
                      req.buyer?.last_name || ""
                    }`.trim()}
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-4">
            <div className={`${locationBg} rounded-lg p-4 shadow-md`}>
              <div className="flex items-center gap-2 mb-2">
                <svg
                  className={`w-5 h-5 ${locationText}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <p className={`text-lg font-bold ${locationTextBold}`}>
            ...
            <div className={`${meetingBg} rounded-lg p-4 shadow-md`}>
              ...
              <p className={`text-lg font-bold ${meetingTextBold}`}>
                {meetingDate}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 justify-end pt-2">
            ...
            <button
              ...
              onClick={() =>
                navigate(
                  `/app/scheduled-purchases/mark-completed/${req.request_id}`
                )
              }
              ...
              className={`px-4 py-2 text-sm font-medium rounded-lg border border-green-500 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20 ${
                !canUseActionButtons ? "opacity-30 cursor-not-allowed" : ""
              }`}
            >
            ...
            <button
              ...
              onClick={() =>
                navigate(
                  `/app/scheduled-purchases/report-issue/${req.request_id}`
                )
              }
              disabled={!canUseActionButtons}
    ...
    return (
      <div>
        ...
        <div className="space-y-4">
          {requests.map((req) => (
            <PurchaseCard
              key={`${perspective}-${req.request_id}`}
              req={req}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-300">
            Review scheduled meetup requests. Accept to confirm the plan or
            decline if it does not work for you.
          </p>
        </div>
        ...
        {loading ? (
          ...
          activeSellerRequests.length === 0 &&
          pastBuyerRequests.length === 0 &&
          pastSellerRequests.length === 0 ? (
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const res = await fetch(
                          `${API_BASE}/scheduled-purchases/cancel.php`,
                          {
                            method: "POST",
                            headers: {
                              "Content-Type": "application/json",
