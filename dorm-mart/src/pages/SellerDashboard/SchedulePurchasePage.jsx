...
function SchedulePurchasePage() {
  ...
  const [selectedListingId, setSelectedListingId] = useState(
    navState?.productId ? String(navState.productId) : ""
  );
  const [selectedConversationId, setSelectedConversationId] = useState(
    navState?.convId ? String(navState.convId) : ""
  );
    ...
    async function loadListings() {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/seller-dashboard/manage_seller_listings.php`,
          {
            method: "POST",
            headers: {
  ...
  const combineDateTime = () => {
    ...
    const dateTimeString = `${year}-${String(month).padStart(2, "0")}-${String(
      day
    ).padStart(2, "0")}T${String(hour24).padStart(2, "0")}:${meetingMinute}:00`;
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-300">
            Coordinate a meetup with a buyer you are chatting with. They will
            confirm on their side and share the provided 4-character code at the
            exchange.
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          {navState && (navState.convId || navState.productId) && (
            <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              <p className="text-sm text-blue-800 dark:text-blue-200">
                Some fields have been prefilled from your chat conversation.
              </p>
            </div>
          )}
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              >
                <option value="">
                  {loading ? "Loading your listings..." : "Choose a listing"}
                </option>
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">
                Select Buyer (Active Conversations)
              </label>
              <select
                value={
                  navState?.convId
                ...
                className="w-full bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 dark:disabled:bg-gray-700 disabled:cursor-not-allowed"
                disabled={
                  conversationOptions.length === 0 || !!navState?.convId
                }
              >
                <option value="">
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className={`flex-1 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                      isTrade ? "opacity-50 cursor-not-allowed" : ""
                    }`}
                  />
                ...
                {negotiatedPrice.trim() &&
                  ...
                  (() => {
                    ...
                    if (
                      ...
                      negotiatedPriceValue > listedPriceValue
                    ) {
                      return (
                        <p className="mt-2 text-sm text-red-600 dark:text-red-400">
