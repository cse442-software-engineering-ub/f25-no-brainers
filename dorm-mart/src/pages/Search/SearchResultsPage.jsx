  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const r = await fetch(`${API_BASE}/search/get_search_items.php`, {
          method: "POST",
                      ...
                      <div className="grid grid-cols-[4.5rem,1fr,6.5rem] md:grid-cols-[6rem,1fr,8rem] gap-3 items-center">
                        ...
                        <div className="flex flex-col items-end gap-1">
                          <div className="text-base md:text-lg font-bold text-gray-900 dark:text-gray-100">
                            {formatCurrency(it.price) ?? "$0.00"}
                          </div>
                          {it.status &&
                          String(it.status).toUpperCase() !== "AVAILABLE" ? (
                            <span
